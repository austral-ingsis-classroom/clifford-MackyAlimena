...
public class FileSystemUpdater {
  ...
  private static DirectoryNode propagateUpdate(DirectoryNode node) {
    ...
    if (parent == null) {
      ...
      System.out.println(
          "Root children now: " + node.getChildren().stream().map(FileSystemNode::name).toList());
      return node;
    }
    ...
    System.out.println(
        "Parent children before: "
            + parent.getChildren().stream().map(FileSystemNode::name).toList());
    System.out.println(
        "Parent children after update: "
            + updatedParent.getChildren().stream().map(FileSystemNode::name).toList());
